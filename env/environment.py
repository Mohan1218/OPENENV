...
class BaseTaskEnv:
    ...
    def step(self, action: Dict[str, Any]) -> Tuple[Dict[str, Any], float, bool, Dict[str, Any]]:
        try:
            ...
            # Ensure all return values have correct types
            return (
                next_obs or {},
                float(reward),
                bool(done),
                {
                    ...
                    "difficulty": str(self.difficulty.value),
                    "step": int(self.index),
                    "episode_length": int(len(self.data)),
                    "total_score": float(self.total_score),
                    "grade": grade or {},
                    "explanation_breakdown": explanation_breakdown or {},
                    "history_length": int(len(self.history)),
                    "difficulty_adapted": bool(reward < 0.3 or reward > 0.7),
                    "risk_level": str(risk_level),
                    "time_decay_applied": bool(time_penalty > 0),
                } or {}
            )
        
        except Exception as e:
            # Safety wrap: Prevent crashes, return graceful error
            import traceback
